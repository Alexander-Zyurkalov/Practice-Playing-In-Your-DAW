    ...
    return
            {
                    ...
                    std::make_unique<juce::AudioParameterFloat>("windowHeight", "Window Height", 0.0f, 1440.0f, 600.0f),
            };
}
...
int AudioPluginAudioProcessor::getWindowHeight() const
{
    return static_cast<int>(std::round(*parameters.getRawParameterValue("windowHeight")));
}

void AudioPluginAudioProcessor::setWindowHeight(int height)
{
    parameters.getParameter("windowHeight")->setValueNotifyingHost(static_cast<float>(height) / 1440.0f);
}
